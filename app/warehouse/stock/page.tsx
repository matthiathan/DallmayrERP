...
import { ErpMetricCard, ErpMetricGrid, ErpPage, ErpPageHeader, ErpPanel } from '@/components/ui/ErpLayout';
...
export default function WarehouseStockPage() {
  ...
  return (
    <AppShell>
      <ErpPage className="erp-list-workspace erp-stock-register-workspace" variant="list">
      <ErpPageHeader actions={<><Link className="button" href="/warehouse/purchasing">Purchase orders</Link><Link className="button secondary" href="/warehouse/locations">Locations</Link><Link className="button secondary" href="/warehouse/ledger">Movement ledger</Link></>} description="Phone scanning, visual item records, location balances, purchasing, receiving, issuing, transfers and low-stock control." eyebrow="Stock control centre" title="Warehouse Inventory" />
      {error ? <div className="error">{error}</div> : null}
      {success ? <div className="success">{success}</div> : null}

      <ErpMetricGrid className="spatial-kpi-grid" columns={4}><ErpMetricCard label="Active items" value={items.filter((item) => item.is_active).length} /><ErpMetricCard label="Low-stock alerts" tone={lowStockItems.length ? 'danger' : 'good'} value={lowStockItems.length} /><ErpMetricCard label="Total equivalent units" value={totalEquivalentUnits} /><ErpMetricCard label="Estimated stock value" value={`R ${stockValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`} /></ErpMetricGrid>
      ...
      <ErpPanel description="Maintain item identity, barcodes, supplier, pricing and reorder settings. Quantities are controlled only through audited transactions." title="Create or update item master"><form className="grid" onSubmit={saveItemMaster}><div className="form-grid"><label>Stock name<input required value={form.stock_name} onChange={(event) => setForm({ ...form, stock_name: event.target.value })} /></label><label>SKU<input value={form.sku} onChange={(event) => setForm({ ...form, sku: event.target.value })} /></label><label>Category<input value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} /></label></div><div className="form-grid"><label>Item barcode<input required value={form.item_barcode} onChange={(event) => setForm({ ...form, item_barcode: event.target.value })} /></label><label>Box barcode<input value={form.box_barcode} onChange={(event) => setForm({ ...form, box_barcode: event.target.value })} /></label><label>Items per box<input min="1" type="number" value={form.items_per_box} onChange={(event) => setForm({ ...form, items_per_box: Number(event.target.value) })} /></label></div><div className="form-grid"><label>Supplier<input value={form.supplier_name} onChange={(event) => setForm({ ...form, supplier_name: event.target.value })} /></label><label>Default location label<input value={form.warehouse_location} onChange={(event) => setForm({ ...form, warehouse_location: event.target.value })} /></label><label>Reorder level<input min="0" type="number" value={form.reorder_level} onChange={(event) => setForm({ ...form, reorder_level: Number(event.target.value) })} /></label><label>Preferred reorder quantity<input min="0" type="number" value={form.preferred_reorder_quantity} onChange={(event) => setForm({ ...form, preferred_reorder_quantity: Number(event.target.value) })} /></label><label>Unit cost<input min="0" step="0.01" type="number" value={form.unit_cost} onChange={(event) => setForm({ ...form, unit_cost: event.target.value })} /></label><label>Sales price<input min="0" step="0.01" type="number" value={form.sales_price} onChange={(event) => setForm({ ...form, sales_price: event.target.value })} /></label></div><label>Description<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label>Notes<textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label><button className="button" disabled={saving || !form.stock_name.trim() || !form.item_barcode.trim()} type="submit">{saving ? 'Saving item...' : 'Save item master'}</button></form></ErpPanel>
