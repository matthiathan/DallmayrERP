...
function sessionFixture() {
  ...
  const payload = Buffer.from(JSON.stringify({
    sub: authUserId,
    email: 'machine-command@example.com',
    role: 'authenticated',
  ...
  return {
    ...
    refresh_token: 'machine-command-refresh-token',
    token_type: 'bearer',
    expires_in: 7200,
...
async function installMock(page) {
  ...
  await page.route(`${supabaseOrigin}/**`, async (route) => {
    ...
    if (url.pathname === '/rest/v1/machines') return route.fulfill(jsonResponse({
      ...
      site_id: 'site-1',
      asset_tag: 'AST-001',
      serial_number: 'BEL-001',
      machine_barcode: 'QR-BEL-001',
      machine_name: 'Belluno 01',
      model: 'Belluno',
      status: 'active',
      current_custodian: null,
      manufacturer: 'Dallmayr',
      condition: 'good',
    ...
    if (url.pathname === '/rest/v1/telemetry_devices') return route.fulfill(jsonResponse({
      ...
      status: 'active',
      profile_id: 'belluno-mdb',
      firmware_version: '6.8.41',
      wifi_rssi: -58,
      cellular_csq: 24,
      ...
      hardware_uid: 'ESP32S3-TEST-001',
      reported_machine_serial: 'BEL-001',
      machine_link_status: 'linked',
      machine_link_method: 'automatic',
      applied_config: { mode: 'live' },
    }));

    if (url.pathname === '/rest/v1/customer_sites') return route.fulfill(jsonResponse({
      id: 'site-1', site_name: 'Johannesburg Test Site', address: '1 Test Road, Johannesburg', latitude: -26.2041, longitude: 28.0473,
    }));
    if (url.pathname === '/rest/v1/customers') return route.fulfill(jsonResponse({
      id: 'customer-1', customer_name: 'Test Customer', customer_code: 'TC001',
    }));
    ...
    if (url.pathname === '/rest/v1/rpc/get_telemetry_data_usage') return route.fulfill(jsonResponse({
      ...
      last_reported_at: '2026-09-09T05:55:00.000Z',
    ...
    if (url.pathname === '/rest/v1/rpc/get_telemetry_prepaid_balances') return route.fulfill(jsonResponse({
      device_id: 'device-1', remaining_bytes: 524_288_000, query_status: 'ok', alert_level: 'ok', checked_at: '2026-09-09T05:55:00.000Z', is_stale: false,
    }));
...
test('machine command center is visual, interactive and keeps top-point tooltips visible on mobile', async ({ browser }) => {
  const context = await browser.newContext({ viewport: { width: 390, height: 844 }, hasTouch: true, isMobile: true });
  const page = await context.newPage();
  await installMock(page);
  await page.goto(`${baseURL}/machines/machine-1`, { waitUntil: 'domcontentloaded' });

  const commandCenter = page.locator('[data-machine-command-center="v1"]');
  await expect(commandCenter).toBeVisible({ timeout: 20_000 });
  await expect(page.getByRole('heading', { name: 'Belluno 01', level: 1 })).toBeVisible();
  await expect(commandCenter.getByText('Items sold', { exact: true })).toBeVisible();
  await expect(commandCenter.getByText('Lifetime cups', { exact: true })).toBeVisible();
  await expect(commandCenter.getByText('30-day data', { exact: true })).toBeVisible();
  await expect(commandCenter.getByRole('img', { name: /Cellular signal high/i })).toBeVisible();

  const line = commandCenter.locator('[data-chart-interactive="line"]');
  const bars = commandCenter.locator('[data-chart-interactive="bar"]');
  const donut = commandCenter.locator('[data-chart-interactive="donut"]');
  await expect(line).toBeVisible();
  await expect(bars).toBeVisible();
  await expect(donut).toBeVisible();

  const topPoint = line.locator('circle[role="button"]').last();
  await topPoint.click();
  await expect(topPoint).toHaveAttribute('aria-pressed', 'true');
  const tooltip = line.getByRole('status');
  await expect(tooltip).toBeVisible();
  await expect(tooltip).toHaveAttribute('data-tooltip-placement', 'below');
  await expect(tooltip).toContainText('10 items');
