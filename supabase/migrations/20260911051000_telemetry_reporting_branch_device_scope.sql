...
begin
  ...
  device_scope as (
    select
      ...
      d.machine_id,
      d.status,
      lower(coalesce(m.branch, cs.branch, 'unassigned')) as branch
    from public.telemetry_devices d
    left join public.machines m on m.id = d.machine_id
    left join public.customer_sites cs on cs.id = coalesce(d.site_id, m.site_id)
    where d.status = 'active'
      and (v_branch = 'all' or lower(coalesce(m.branch, cs.branch, '')) = v_branch)
  )
  select jsonb_build_object(
    ...
    'availability', jsonb_build_object(
      'production_rows', (
        select count(*)
        from public.telemetry_daily_item_sales s
        where s.sales_date between v_from and v_today
          and (v_branch = 'all' or lower(s.branch) = v_branch)
      ),
      'simulation_rows', (
        select count(*)
      ...
      'active_simulation_devices', (
        select count(*)
        from public.telemetry_machine_state ms
        join device_scope ds on ds.id = ms.device_id
        where ms.simulation_mode
      )
    ),
    'summary', jsonb_build_object(
      ...
      'online_devices', (
        ...
        cross join lateral public.get_telemetry_connectivity_state(ds.id) c
        where c.communication_status = 'online'
      ),
      'offline_devices', (
        select count(*)
        from device_scope ds
        cross join lateral public.get_telemetry_connectivity_state(ds.id) c
        where c.communication_status = 'offline'
      ),
