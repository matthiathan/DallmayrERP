...
declare
  ...
  v_rec_id uuid := null;
  v_rec_model_key text := null;
  v_rec_display_name text := null;
  v_rec_button_count integer := null;
...
begin
  ...
  if v_device.id is not null then
    ...
    select mp.id, mp.model_key, mp.display_name, mp.button_count,
      case
    ...
    into v_rec_id, v_rec_model_key, v_rec_display_name, v_rec_button_count, v_score
    from public.machine_model_profiles mp
    order by 5 desc, mp.display_name
    limit 1;

    v_score := coalesce(v_score, 0);
    if v_score < 70 then
      v_rec_id := null;
      v_rec_model_key := null;
      v_rec_display_name := null;
      v_rec_button_count := null;
      v_score := 0;
    ...
    if coalesce(v_device.profile_assignment_method, 'automatic') = 'manual' then
      ...
      v_effective_profile := v_rec_model_key;
    end if;
  ...
  return jsonb_build_object(
    ...
    'recommended_profile', case when v_rec_id is null then null else jsonb_build_object(
      'id', v_rec_id,
      'model_key', v_rec_model_key,
      'display_name', v_rec_display_name,
      'button_count', v_rec_button_count,
      'score', v_score,
      'reason', v_reason
    ) end,
