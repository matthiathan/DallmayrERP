-- Correct the production telemetry fleet join: telemetry_devices.profile_id stores
-- the profile model_key (text), while machine_model_profiles.id is UUID.
-- Joining on model_key keeps fresh environments aligned with production.
...
begin
  ...
  with active_devices as (
    select distinct on (d.machine_id)
      d.machine_id,d.id device_id,d.device_code,
      coalesce(ms.telemetry_mode,ep.policy->>'mode','live') telemetry_mode,
      coalesce(ms.machine_status,'unknown') machine_status,
      d.last_transport,d.wifi_rssi,d.cellular_csq,d.cellular_operator,d.firmware_version,d.last_seen_at,d.last_heartbeat_at,
      ...
      case when d.profile_id is null then 'unmatched'
        ...
        else 'configured' end profile_status
    from public.telemetry_devices d
    left join public.telemetry_machine_state ms on ms.device_id=d.id
    left join public.machine_model_profiles mp on mp.model_key=d.profile_id
    ...
    where f.cleared_at is null and f.machine_id is not null and d.telemetry_region=v_region
    ...
    select m.id,m.branch,m.site_id,m.serial_number,m.machine_barcode,m.asset_tag,m.machine_name,m.model,m.status asset_status,m.current_custodian,m.manufacturer,
      ...
      case when d.device_id is null then 'unlinked'
    ...
    select f.*
    from filtered f
    order by case when f.fault_count>0 then 0 else 1 end,
      case f.profile_status when 'pending' then 0 when 'unmatched' then 1 else 2 end,
      lower(coalesce(f.machine_name,f.model,f.serial_number,f.asset_tag,f.id::text)),f.id
    limit v_limit offset v_offset
  ), fleet_summary as (
    select count(*)::bigint fleet_total,
      ...
      count(*) filter(where device_id is not null and profile_status in ('unmatched','pending'))::bigint profile_attention
    from base
  )
  select jsonb_build_object(
    ...
    'summary',jsonb_build_object(
      'online',fs.online,'delayed',fs.delayed,'offline',fs.offline,'never',fs.never,'unlinked',fs.unlinked,
