-- Extend the existing region-scoped fleet RPC with decoder-profile rollout
-- filters and summary counts. Keep one authoritative resolver path so rollout
-- dashboards do not reimplement profile matching in the browser.
...
begin
  ...
  with active_devices as (
    ...
    select
      ...
      d.reported_machine_profile_fingerprint,d.reported_machine_revision,d.reported_machine_identity_source,d.reported_machine_identity_at,
    ...
    select
      ...
      count(*) filter(where device_id is not null)::bigint linked_devices,
      count(*) filter(where device_id is not null and profile_status='configured')::bigint profile_configured,
      count(*) filter(where device_id is not null and profile_status='pending')::bigint profile_pending,
      count(*) filter(where device_id is not null and profile_status='ambiguous')::bigint profile_ambiguous,
      count(*) filter(where device_id is not null and profile_status='unmatched')::bigint profile_unmatched,
      count(*) filter(where device_id is not null and profile_assignment_method='manual')::bigint profile_manual,
      count(*) filter(where device_id is not null and coalesce(profile_assignment_method,'automatic')='automatic')::bigint profile_automatic,
      count(*) filter(where device_id is not null and profile_status in ('configured','pending'))::bigint profile_resolved,
      count(*) filter(where device_id is not null and profile_status in ('ambiguous','unmatched'))::bigint profile_unresolved,
  ...
  select jsonb_build_object(
    ...
    'summary',jsonb_build_object(
      ...
      'linked_devices',fs.linked_devices,
      'profile_configured',fs.profile_configured,
      'profile_pending',fs.profile_pending,
      'profile_ambiguous',fs.profile_ambiguous,
      'profile_unmatched',fs.profile_unmatched,
      'profile_manual',fs.profile_manual,
      'profile_automatic',fs.profile_automatic,
      'profile_resolved',fs.profile_resolved,
      'profile_unresolved',fs.profile_unresolved,
