/*
  Dallmayr South Africa - Telemetry V6.8.14 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.14-esp32s3-air780eu-bounded-http-action";
...
// V1180 defaults to 120 seconds, so the old 65-second MCU wait interrupted an
// active request and made the following HTTPTERM fail with +CME ERROR: 3.
...
String readAir780HttpAction(uint32_t timeoutMs, bool& bearerDeactivated) {
  String response;
  ...
  while (millis() - started < timeoutMs) {
    while (CellSerial.available()) {
      ...
      // Keep waiting after the immediate command OK. HTTPACTION completes only
      // when its asynchronous result line arrives. This also consumes and
      // records bearer-loss URCs that may precede the final HTTP status.
      if (!bearerDeactivated &&
          (response.indexOf("+SAPBR 1: DEACT") >= 0 ||
           response.indexOf("+CGEV: NW PDN DEACT") >= 0 ||
           response.indexOf("+PDP: DEACT") >= 0)) {
        bearerDeactivated = true;
      }

      int actionPos = response.lastIndexOf("+HTTPACTION:");
      if (actionPos >= 0 && response.indexOf('\n', actionPos) >= 0) return response;
      if (response.indexOf("\r\nERROR\r\n") >= 0 ||
          response.indexOf("+CME ERROR:") >= 0) return response;
    }
    delay(2);
  }
  return response;
...
bool restartAir780AfterHttpStall() {
  ...
  // fault. A successful HTTPACTION clears this guard for future recovery.
  ...
  // active V1180 HTTPACTION. The next maintainCellular() pass will rebuild the
...
bool airHttpPost(const char* url, const String& json, String& responseBody, int& statusCode, bool withDeviceAuth = true) {
  ...
  String action = readAir780HttpAction(AIR780_HTTP_ACTION_WAIT_MS, bearerDeactivated);
  if (action.indexOf("+HTTPACTION:") < 0) {
    Serial.print(F("No complete HTTPACTION result after the configured modem timeout ("));
  ...
  if (bearerDeactivated) {
    Serial.println(F("Air780E bearer deactivated during HTTPACTION; cellular will reconnect after this result."));
    cellReady = false;
  }
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.14 BOUNDED HTTP ACTION DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.14 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
