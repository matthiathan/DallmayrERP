/*
  Dallmayr South Africa - Telemetry V6.8.11 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.11-esp32s3-air780eu-ssl-busy-recovery";
...
bool configureAir780TlsContext() {
  ...
  // OpenLuat assigns SSL context 153 to the HTTP/HTTPS service. Configure it
  // before HTTPINIT, matching the module's documented HTTPS command order.
  ...
  // Keep the modem's own TLS deadline below the MCU's 65-second HTTPACTION
  // wait. V6.8.9 used 120 seconds, so the MCU timed out first and tried to
  // initialize another HTTP session while the previous negotiation was busy.
...
bool beginAir780HttpsSession() {
  ...
  if (!configureAir780TlsContext()) {
    ...
    // A stale V1180 HTTPACTION can reject HTTPSSL=1 with +CME ERROR: 3 before
    // HTTPINIT is reached. Reset the separate modem here as well; resetting the
    // ESP32 alone does not clear the Air780EU HTTP service.
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.11 SSL-BUSY RECOVERY + TLS CONTEXT DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.11 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
