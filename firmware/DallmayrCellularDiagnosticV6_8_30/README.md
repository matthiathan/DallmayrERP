# Air780EU Cellular Diagnostic V6.8.30
...
1. Flash `DallmayrCellularDiagnosticV6_8_30.ino`.
...
## V6.8.30 recovery change

V6.8.29 could falsely stop after a successful `+++` / `ATH` recovery because its AT parser treated the echoed `AT` line as an idle-complete response after 300 ms, before the modem's final `OK` arrived.

V6.8.30:
- waits for an actual terminal result instead of an echoed command;
- sends an AT wake burst before assuming the modem is unavailable;
- retries AT five times after `ATH`;
- sends `AT+CSCLK=0` once command mode is confirmed so the diagnostic runs with modem sleep disabled.
