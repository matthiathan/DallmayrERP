/*
  Dallmayr South Africa - Telemetry V6.8.32 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.32-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.32 deliberately uses
...
bool initializeCellular() {
  ...
  // V6.8.31 diagnostic proved Vodacom PPP succeeds with CID1 left active
  // and its network-assigned address preserved. Do not force CGACT=0,1 here.
  // SAPBR is still closed so the modem's internal HTTP stack does not compete.
...
bool startCellularPpp() {
  ...
  // Preserve the active Vodacom CID1/PDP address proven by V6.8.31.
  // Only close SAPBR; do not deactivate CID1 before ATD*99#.
  cellCommand("AT+SAPBR=0,1", "OK", 3000, true);
  delay(250);
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.32 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.32 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/MANUAL-PPPOS-VERIFIED"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
