/*
  Dallmayr South Africa - Telemetry V6.8.22 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.22-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.22 deliberately uses
...
bool startCellularPpp() {
  ...
  // The legacy SAPBR bearer was only needed by the modem's own HTTP engine.
  // Close it before handing UART1 to esp-modem/lwIP PPP.
  ...
  Serial.println(F("Air780EU attached; dialing PPP data mode with generic ATD*99#."));
  if (!PPP.mode(ESP_MODEM_MODE_DATA)) {
  ...
  if (!PPP.hasIP()) {
    Serial.println(F("Air780EU PPP negotiation timed out without an IP address."));
    stopCellularPpp(true);
    cellModemRegistered = false;
    return false;
  }
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.22 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.22 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/PPP-CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
