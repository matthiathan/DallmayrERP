/*
  Dallmayr South Africa - Telemetry V6.8.13 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.13-esp32s3-air780eu-compact-headers";
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.13 COMPACT HTTP HEADERS DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.13 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
