/*
  Dallmayr South Africa - Telemetry V6.8.16 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.16-esp32s3-vodacom-prepaid-balance";
...
void queryVodacomPrepaidBalance() {
  ...
  String support = cellQueryText("AT+CUSD=?", 4000);
  if (support.indexOf("ERROR") >= 0) {
    prepaidBalanceStatus = "unsupported";
    prepaidBalanceError = "Air780EU firmware does not expose AT+CUSD";
  } else {
    cellCommand("AT+CUSD=2", "OK", 2500, true);
    cellDrain();
    String ussdCode = strlen(policy.prepaidBalanceUssdCode)
      ? String(policy.prepaidBalanceUssdCode) : String("*135*500#");
    ...
    if (response.indexOf("+CUSD:") >= 0) {
      ...
      prepaidBalanceError = response.indexOf("+CME ERROR:") >= 0
        ? "Air780EU rejected the Vodacom USSD request"
        : "Vodacom USSD balance request timed out";
    }
    cellCommand("AT+CUSD=2", "OK", 2500, true);
  }
...
void addPrepaidBalanceMetadata(JsonDocument& doc) {
  ...
  balance["ussd_code"] = strlen(policy.prepaidBalanceUssdCode)
    ? policy.prepaidBalanceUssdCode : "*135*500#";
...
bool syncRemotePolicy() {
  ...
  copyText(policy.prepaidBalanceUssdCode, sizeof(policy.prepaidBalanceUssdCode),
           prepaidControl["ussd_code"] | "*135*500#");
...
void loadCachedPolicy() {
  ...
  copyText(policy.prepaidBalanceUssdCode, sizeof(policy.prepaidBalanceUssdCode), "*135*500#");
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.16 VODACOM PREPAID BALANCE DB-POLICY NATIVE MDB + DEX ---"));
...
void printHelp() {
  ...
  Serial.println(F("  VODACOM BALANCE   (query prepaid data balance with *135*500#)"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.16 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
