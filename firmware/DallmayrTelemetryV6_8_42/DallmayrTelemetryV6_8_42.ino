/*
  Dallmayr South Africa - Telemetry V6.8.42 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.42-esp32s3-air780eu-mdb-polarity-control";
...
// Keep the MCU wait longer than the modem deadline. V6.8.42 deliberately uses
...
void serviceMdbRawDebugEvents(uint8_t maxFrames = 4) {
  ...
  while (handled < maxFrames && xQueueReceive(mdbRawDebugQueue, &frame, 0) == pdTRUE) {
    ...
    for (uint8_t i = 0; i < frame.count; ++i) {
      if (i) Serial.print(' ');
      if ((frame.modeMask & (1UL << i)) != 0) Serial.print('^');
      if (frame.values[i] < 0x10) Serial.print('0');
      Serial.print(frame.values[i], HEX);
    }
    Serial.println();
  }
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.42 AUTO-PROFILE CUP-COUNTERS DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.42 - PASSIVE MDB + DEX + AUTO-PROFILE EVIDENCE + CUP COUNTERS + WIFI/MANUAL PPPOS"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
