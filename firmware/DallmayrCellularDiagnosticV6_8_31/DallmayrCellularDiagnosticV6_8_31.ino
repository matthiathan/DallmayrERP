/*
  Dallmayr South Africa - Air780EU Cellular Diagnostic V6.8.31
...
bool initializeEsp32NetworkCore() {
  Serial.println(F("[ESP32 NET] Initialising Arduino Network / esp_netif / lwIP core..."));
  if (!Network.begin()) {
    Serial.println(F("[ESP32 NET FAIL] Network.begin() returned false."));
    return false;
  }

  // Network.begin() calls esp_netif_init() in Arduino-ESP32. This creates the
  // lwIP/TCPIP infrastructure used by LOCK_TCPIP_CORE(). V6.8.30 omitted this
  // because Wi-Fi had been removed from the stripped diagnostic, leaving the
  // TCPIP core semaphore null and causing xQueueSemaphoreTake to assert.
  delay(100);
  Serial.println(F("[ESP32 NET PASS] TCP/IP core initialized; raw lwIP PPP is now safe to create."));
  return true;
}
...
void runDiagnostic() {
  Serial.println();
  Serial.println(F("======================================================"));
  Serial.println(F("Dallmayr Air780EU Cellular Diagnostic V6.8.31"));
  Serial.println(F("ONE ATTEMPT ONLY - NO AUTOMATIC RETRY LOOP"));
  Serial.println(F("======================================================"));

  if (!initializeEsp32NetworkCore()) {
    Serial.println(F("[STOP] ESP32 network core did not initialise."));
    return;
  }
  ...
  Serial.println(F("[PPP] ESP32 TCP/IP core was initialized before LOCK_TCPIP_CORE()."));
  if (!createPpp()) return;
