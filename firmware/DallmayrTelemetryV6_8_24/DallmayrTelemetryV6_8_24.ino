/*
  Dallmayr South Africa - Telemetry V6.8.24 DB-POLICY NATIVE MDB + DEX
  ...
  ----------------------------
  - ArduinoJson 7.x
  - Current Arduino-ESP32 3.x (RMT + PPP APIs used by this sketch)
  - Preferences/WiFi/Network/PPP/HTTPClient are supplied by the ESP32 Arduino core.
*/
...
static const char* FIRMWARE_VERSION = "6.8.24-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.24 deliberately uses
...
static const uint32_t PPP_CONNECT_TIMEOUT_MS = 30000UL;
...
void queryVodacomPrepaidBalance() {
  ...
  // Air780EU V1180 can reject the AT+CUSD=? test form even when the executable
  // AT+CUSD command works. Send the SIM-verified request directly instead of
  // treating a failed capability probe as proof that USSD is unsupported.
  cellCommand("AT+CUSD=2", "OK", 2500, true);
  cellDrain();
  String ussdCode = strlen(policy.prepaidBalanceUssdCode)
    ? String(policy.prepaidBalanceUssdCode) : String(DEFAULT_PREPAID_BALANCE_USSD);
  Serial.println(F("Skipping unreliable AT+CUSD=? capability probe."));
  Serial.print(F("Requesting Vodacom prepaid data balance with "));
  Serial.println(ussdCode);
  CellSerial.print("AT+CUSD=1,\"");
  CellSerial.print(ussdCode);
  CellSerial.print("\",15\r\n");

  String response = cellReadUntil(VODACOM_USSD_TIMEOUT_MS, "+CUSD:", "\r\nERROR\r\n",
                                  false, "+CME ERROR:");
  if (response.indexOf("+CUSD:") >= 0) {
    response += cellReadUntil(5000, "\r\n", "\n", false, "+CME ERROR:");
    prepaidBalanceText = extractUssdPayload(response).substring(0, 480);
    uint64_t parsedBytes = 0;
    if (prepaidBalanceText.length() && parseDataBalanceBytes(prepaidBalanceText, parsedBytes)) {
      prepaidBalanceRemainingBytes = parsedBytes;
      prepaidBalanceAvailable = true;
      prepaidBalanceStatus = "ok";
    } else {
      prepaidBalanceStatus = "parse_failed";
      prepaidBalanceError = prepaidBalanceText.length()
        ? "Vodacom response did not contain a recognised KB, MB or GB balance"
        : "Vodacom returned an empty USSD response";
    }
  } else {
    prepaidBalanceStatus = "failed";
    String modemResponse = response;
    modemResponse.replace('\r', ' ');
    modemResponse.replace('\n', ' ');
    while (modemResponse.indexOf("  ") >= 0) modemResponse.replace("  ", " ");
    modemResponse.trim();
    if (modemResponse.length() > 180) modemResponse = modemResponse.substring(modemResponse.length() - 180);
    if (response.indexOf("+CME ERROR:") >= 0 || response.indexOf("\r\nERROR\r\n") >= 0) {
      prepaidBalanceError = "Air780EU rejected the direct Vodacom USSD request";
    } else if (response.indexOf("OK") >= 0) {
      prepaidBalanceError = "Air780EU accepted the USSD request but Vodacom returned no +CUSD response";
    } else {
      prepaidBalanceError = "Vodacom USSD balance request timed out before the modem accepted it";
    }
    if (modemResponse.length()) {
      prepaidBalanceError += ": ";
      prepaidBalanceError += modemResponse;
    }
  }
  cellCommand("AT+CUSD=2", "OK", 2500, true);
  ...
  Serial.print(F("Vodacom prepaid balance query status: "));
  Serial.println(prepaidBalanceStatus);
  if (prepaidBalanceAvailable) {
    char balanceBuffer[24] = {0};
    snprintf(balanceBuffer, sizeof(balanceBuffer), "%llu",
             static_cast<unsigned long long>(prepaidBalanceRemainingBytes));
    Serial.print(F("Vodacom prepaid data remaining: "));
    Serial.print(balanceBuffer);
    Serial.println(F(" B"));
  } else if (prepaidBalanceError.length()) {
    Serial.println(prepaidBalanceError);
  }
}

void servicePrepaidBalance() {
  // PPP data mode owns UART1, so AT/USSD is intentionally deferred while the
  // cellular IP link is carrying production traffic.
  if (pppStarted) return;
  if (!deviceEnrolled() || !cellModemRegistered || !policy.prepaidBalanceEnabled) return;
  uint32_t now = millis();
  uint32_t intervalMinutes = policy.prepaidBalanceCheckIntervalMinutes == 0
    ? 360 : policy.prepaidBalanceCheckIntervalMinutes;
  uint32_t intervalMs = intervalMinutes * 60000UL;
  bool requested = policy.prepaidBalanceDue;
  bool scheduled = lastPrepaidBalanceCheckMs == 0 || now - lastPrepaidBalanceCheckMs >= intervalMs;
  if (!requested && !scheduled) return;
  if (lastPrepaidBalanceCheckMs != 0
      && now - lastPrepaidBalanceCheckMs < PREPAID_BALANCE_FAILED_RETRY_MS) return;
...
bool recoverAir780CommandMode(bool verbose = true) {
  ...
  if (verbose) Serial.println(F("Attempting guarded Air780EU PPP/data-mode escape..."));
  ...
  // Whether ATH returned OK or NO CARRIER, prove command mode with a fresh AT.
  cellDrain();
  CellSerial.print("AT\r\n");
  String probe = cellReadUntil(2500, "\r\nOK\r\n", "\r\nERROR\r\n");
  bool recovered = probe.indexOf("\r\nOK\r\n") >= 0;
  if (verbose) {
    Serial.println(recovered
      ? F("Air780EU command mode recovered.")
      : F("Air780EU command-mode recovery did not obtain AT/OK."));
  }
  return recovered;
}
...
bool initializeCellular() {
  ...
  // Send both accepted spellings before PPP and verify again through esp-modem
  ...
  // PPPoS must own PDP context 1 directly. Do not activate SAPBR here:
  // SAPBR belongs to the modem's internal HTTP/TCP stack and can leave CID 1
  // active on the wrong data path before ATD*99# is issued.
  cellCommand("AT+SAPBR=0,1", "OK", 3000, true);
  cellCommand("AT+CGACT=0,1", "OK", 6000, true);
  delay(500);
...
void stopCellularPpp(bool restoreAt = true) {
  if (pppStarted) {
    // Ask esp-modem to leave data mode first. Some Air780EU V1180 sessions
    // ignore the generic unguarded +++ transition, so raw guarded recovery is
    // performed after PPP.end() as the authoritative cleanup.
...
bool startCellularPpp() {
  ...
  // Ensure the modem's internal bearer is down and CID 1 is inactive before
  // esp-modem takes UART1. The PPP dial itself will activate CID 1.
  cellCommand("AT+SAPBR=0,1", "OK", 3000, true);
  cellCommand("AT+CGACT=0,1", "OK", 6000, true);
  delay(750);
  CellSerial.end();
  ...
  PPP.setApn(pppApn);
  PPP.setPins(CELL_TX_PIN, CELL_RX_PIN, -1, -1, ESP_MODEM_FLOW_CONTROL_NONE);

  Serial.print(F("Starting Air780EU PPPoS on APN: "));
  Serial.println(pppApn);

  if (!PPP.begin(PPP_MODEM_GENERIC, 1, CELL_BAUD)) {
    Serial.println(F("Air780EU PPP driver could not start."));
    pppStarted = false;
    pppOnline = false;
    cellReady = false;
  ...
  // Repeat the PPP-critical setup through the exact DTE that will carry PPP.
  // This eliminates any UART hand-off ambiguity and makes sure command echo is
  // disabled immediately before binary LCP frames begin.
  String echoResult = PPP.cmd("ATE0", 2500);
  Serial.print(F("PPP DTE echo disable: "));
  Serial.println(echoResult.length() ? echoResult : "<no response>");

  String cmEeResult = PPP.cmd("AT+CMEE=2", 2500);
  if (!cmEeResult.length()) Serial.println(F("PPP DTE CMEE setup returned no text."));

  String pdpReset = PPP.cmd("AT+CGACT=0,1", 6000);
  Serial.print(F("PPP CID1 deactivate: "));
  Serial.println(pdpReset.length() ? pdpReset : "<no response>");
  delay(500);

  String pdpConfigCommand = String("AT+CGDCONT=1,\"IP\",\"") + pppApn + "\"";
  String pdpConfig = PPP.cmd(pdpConfigCommand.c_str(), 3500);
  Serial.print(F("PPP CID1 configure: "));
  Serial.println(pdpConfig.length() ? pdpConfig : "<no response>");
  delay(250);

  uint32_t attachedStart = millis();
  while (!PPP.attached() && millis() - attachedStart < 15000UL) {
    delay(100);
  }
  if (!PPP.attached()) {
    Serial.println(F("Air780EU PPP driver could not confirm packet attachment."));
    stopCellularPpp(true);
    cellModemRegistered = false;
    return false;
  }

  Serial.println(F("Air780EU attached; dialing PPP CID1 with generic ATD*99#."));
  if (!PPP.mode(ESP_MODEM_MODE_DATA)) {
    Serial.println(F("Air780EU refused PPP data mode."));
    stopCellularPpp(true);
    cellModemRegistered = false;
    return false;
  }
  ...
  while (!PPP.hasIP() && millis() - ipStart < PPP_CONNECT_TIMEOUT_MS) {
    delay(100);
  }
  if (!PPP.hasIP()) {
    Serial.println(F("Air780EU PPP negotiation timed out without an IP address. CID1/echo preflight was applied before dialing."));
    stopCellularPpp(true);
    cellModemRegistered = false;
    return false;
  }
  ...
  Serial.print(F("Air780EU PPP connected. ESP32 cellular IP: "));
  Serial.println(PPP.localIP());
  Serial.println(F("Cellular HTTPS now uses ESP32 NetworkClientSecure; the modem HTTP/TLS engine is bypassed."));
  return true;
}
...
void maintainCellular() {
  ...
  // When Wi-Fi is the selected primary and is healthy, do not keep a parallel
  // PPP session consuming SIM data. Cellular remains the recovery path.
  ...
  // Give provisioned Wi-Fi its normal startup grace window before using SIM
  // data as recovery.
  ...
  if (pppOnline) {
    ...
    Serial.println(F("Air780EU PPP lost its IP address; reconnecting."));
    stopCellularPpp(true);
    cellModemRegistered = false;
  }
...
bool pppHttpPost(const char* url, const String& json, String& responseBody,
  ...
  responseBody = "";
  statusCode = 0;
  if (!pppOnline || !PPP.hasIP()) return false;
  if (!supabaseAnonKey.length()) return false;
...
bool anyDataTransportReady() {
  return (policy.wifiEnabled && wifiReady()) || (policy.cellularEnabled && pppOnline && PPP.hasIP());
}
...
bool sendDocumentToIngest(JsonDocument& doc) {
  ...
  for (uint8_t i = 0; i < 2; ++i) {
    ...
    if (strcmp(t, "cellular") == 0 && (!pppOnline || !PPP.hasIP())) continue;
...
bool sendDocumentToIngestViaTransport(JsonDocument& doc, const char* transport) {
  ...
  if (isCellular && (!pppOnline || !PPP.hasIP())) return false;
...
bool runVodacomSimDataTest() {
  ...
  if (!pppOnline) {
    Serial.println(F("Cellular PPP is not ready; attempting to connect now."));
    ensureCellularPpp();
  }
  if (!pppOnline || !PPP.hasIP()) {
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.24 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void printHelp() {
  ...
  Serial.println(F("  CELL TEST          (establish/test Air780EU PPP cellular IP)"));
  ...
  Serial.println(F("  VODACOM BALANCE   (query prepaid data balance with *111*502#)"));
...
void handleConsoleLine(String line) {
  ...
  if (line.equalsIgnoreCase("VODACOM BALANCE")) {
    if (pppStarted) {
      Serial.println(F("Vodacom USSD is deferred while PPP is active. The current V1180 modem also rejects the direct *111*502# USSD request."));
      return;
    }
    if (!cellModemRegistered) {
      Serial.println(F("Cellular modem is not registered; connecting the Air780EU now."));
      lastCellAttemptMs = millis();
      cellModemRegistered = initializeCellular();
    }
    if (!cellModemRegistered) {
      Serial.println(F("Vodacom balance query failed because the Air780EU could not register."));
      prepaidBalanceAvailable = false;
      prepaidBalanceStatus = "modem_not_registered";
      prepaidBalanceError = "Air780EU cellular registration failed before the Vodacom USSD request";
      prepaidBalanceReportPending = true;
      heartbeatUploadRequested = true;
    } else {
      Serial.println(F("Air780EU registered; starting the Vodacom balance query."));
      queryVodacomPrepaidBalance();
    }
    return;
  }
  ...
  if (line.equalsIgnoreCase("CELL PPP STATUS")) {
    ...
    if (pppOnline) { Serial.print(F(" IP=")); Serial.print(PPP.localIP()); }
    Serial.println();
    return;
  }
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.24 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/PPP-CELLULAR-RECOVERY"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
...
#endif
  Serial.println(F("Power-on flow: DB transport policy -> Wi-Fi when primary -> Air780EU PPP recovery/primary -> ESP32 TLS -> Supabase."));
