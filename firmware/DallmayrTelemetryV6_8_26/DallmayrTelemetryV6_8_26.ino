/*
  Dallmayr South Africa - Telemetry V6.8.26 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.26-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.26 deliberately uses
...
void destroyManualAirPpp() {
  ...
  if (airPppPcb != nullptr) {
    // ESP-IDF enables raw-lwIP thread-safety assertions. PPP lifecycle calls
    // from the Arduino task must hold the global TCP/IP core mutex.
    LOCK_TCPIP_CORE();
...
bool startCellularPpp() {
  ...
  // Prepare lwIP before ATD*99#. ESP-IDF's lwIP port requires raw PPP API
  // callers outside the TCP/IP task to hold LOCK_TCPIP_CORE().
  Serial.println(F("Creating lwIP PPPoS control block under TCP/IP core lock."));
  LOCK_TCPIP_CORE();
  ...
  if (!airPppHasIp()) {
    Serial.print(F("Manual Air780EU PPP negotiation failed. lwIP error="));
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.26 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.26 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/MANUAL-PPPOS-CORELOCK"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
