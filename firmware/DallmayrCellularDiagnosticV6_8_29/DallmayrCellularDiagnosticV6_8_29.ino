/*
  Dallmayr South Africa - Air780EU Cellular Diagnostic V6.8.29
  ...
  while (millis() - start < timeoutMs) {
    while (CellSerial.available()) {
      ...
      if (stopOnTerminal) {
        if (out.indexOf("\r\nOK\r\n") >= 0 ||
            out.indexOf("\r\nERROR\r\n") >= 0 ||
            out.indexOf("+CME ERROR:") >= 0 ||
            out.indexOf("NO CARRIER") >= 0 ||
            out.indexOf("CONNECT") >= 0) {
          return out;
        }
      }
    }
    if (out.length() && millis() - lastByte > 300) break;
    delay(2);
  }
  return out;
}
...
bool probeAt() {
  String response = atQuery("AT", 1800);
  return response.indexOf("\r\nOK\r\n") >= 0;
}
...
void runDiagnostic() {
  Serial.println();
  Serial.println(F("======================================================"));
  Serial.println(F("Dallmayr Air780EU Cellular Diagnostic V6.8.29"));
  Serial.println(F("ONE ATTEMPT ONLY - NO AUTOMATIC RETRY LOOP"));
  Serial.println(F("======================================================"));

  if (!probeAt()) {
    Serial.println(F("[INFO] AT probe failed; attempting PPP/data-mode recovery."));
