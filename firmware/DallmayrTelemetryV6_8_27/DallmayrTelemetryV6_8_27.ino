/*
  Dallmayr South Africa - Telemetry V6.8.27 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.27-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.27 deliberately uses
...
static const char* VODACOM_PPP_USERNAME = "guest";
...
#if PAP_SUPPORT
  Serial.println(F("PPP authentication configured: PAP user=guest password=<blank>."));
#else
...
bool startCellularPpp() {
  ...
  // Re-apply the carrier profile before each session in case policy changes.
  LOCK_TCPIP_CORE();
#if PAP_SUPPORT
  ppp_set_auth(
    airPppPcb,
  ...
  if (!airPppHasIp()) {
    Serial.print(F("Manual Air780EU PPP negotiation failed. phase="));
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.27 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
  ...
  Serial.println(F("Cellular PPP auth: PAP user=guest password=<blank>"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.27 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/MANUAL-PPPOS-PAP-REUSE"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
