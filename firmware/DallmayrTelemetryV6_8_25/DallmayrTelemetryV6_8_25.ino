/*
  Dallmayr South Africa - Telemetry V6.8.25 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.25-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.25 deliberately uses
...
bool airPppHasIp() {
  if (!pppOnline || airPppPcb == nullptr) return false;
  const ip4_addr_t* ip = netif_ip4_addr(&airPppNetif);
  return netif_is_up(&airPppNetif) && ip != nullptr && !ip4_addr_isany_val(*ip);
}

String airPppIpText() {
  if (!airPppHasIp()) return "";
  char value[20] = {0};
  ip4addr_ntoa_r(netif_ip4_addr(&airPppNetif), value, sizeof(value));
  return String(value);
}
...
void destroyManualAirPpp() {
  airPppRxTaskRun = false;
  uint32_t waitStarted = millis();
  while (airPppRxTaskHandle != nullptr && millis() - waitStarted < 1000UL) {
    delay(10);
  }
...
bool startCellularPpp() {
  ...
  // Ensure the modem's internal IP bearer does not own CID1.
  cellCommand("AT+SAPBR=0,1", "OK", 3000, true);
  cellCommand("AT+CGACT=0,1", "OK", 6000, true);
  delay(750);
  ...
  if (airPppPcb == nullptr) {
    Serial.println(F("lwIP could not create the Air780EU manual PPPoS control block."));
    recoverAir780CommandMode(false);
    cellModemRegistered = false;
    return false;
  }
  ...
  if (!airPppHasIp()) {
    Serial.print(F("Manual Air780EU manual PPP negotiation failed. lwIP error="));
  ...
  Serial.print(F("Manual Air780EU manual PPP connected. ESP32 cellular IP: "));
...
void maintainCellular() {
  ...
  if (pppOnline || pppStarted) {
    ...
    Serial.println(F("Manual Air780EU manual PPP link is down; recovering command mode."));
    stopCellularPpp(true);
    cellModemRegistered = false;
  }
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.25 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.25 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/MANUAL-PPPOS-CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
