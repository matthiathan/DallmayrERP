/*
  Dallmayr South Africa - Telemetry V6.8.23 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.23-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.23 deliberately uses
...
void stopCellularPpp(bool restoreAt = true) {
  ...
  cellReady = false;
  cellularModemUsageAvailable = false;
  if (restoreAt) restoreCellAtUart();
}
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.23 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.23 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/PPP-CELLULAR-CID1"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
