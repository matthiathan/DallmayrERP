/*
  Dallmayr South Africa - Telemetry V6.8.20 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.20-esp32s3-air780eu-ussd-auto-register";
...
void servicePrepaidBalance() {
  if (!deviceEnrolled() || !cellReady || !policy.prepaidBalanceEnabled) return;
  uint32_t now = millis();
...
bool airHttpPost(const char* url, const String& json, String& responseBody, int& statusCode, bool withDeviceAuth = true) {
  ...
  // Keep Supabase gateway JWT verification and the independent per-device
  // credential, but send one compact documented USERDATA block. V1180 rejected
  // the repeated USER_DEFINED form with +CME ERROR: 3 before HTTP transmission.
  if (!setAir780CompactHttpHeaders(withDeviceAuth)) {
    endAir780HttpSession(false);
    return false;
  }

  // V1180 accepted HTTPDATA but repeatedly returned 408 before any request
  // reached Supabase. Use the extended streaming path, but do not require the
  // +HTTPEXPOST URC to arrive before issuing AT+HTTPEXPOST. The published
  // Air780E flow shows the URC, while field V1180 units can acknowledge
  // HTTPEXACTION with OK and omit/delay that unsolicited line.
  Serial.print(F("Air780EU extended HTTP POST body length: "));
  Serial.println(json.length());
  cellDrain();
  CellSerial.print("AT+HTTPEXACTION=1,");
  CellSerial.print(json.length());
  CellSerial.print("\r\n");

  String actionStart = cellReadUntil(5000, "\r\nOK\r\n", "\r\nERROR\r\n",
                                     false, "+CME ERROR:", "+HTTPEXPOST");
  if (actionStart.indexOf("\r\nERROR\r\n") >= 0 ||
      actionStart.indexOf("+CME ERROR:") >= 0) {
    Serial.println(F("Air780EU rejected HTTPEXACTION before POST data."));
    endAir780HttpSession(true);
    return false;
  }

  bool actionAccepted = actionStart.indexOf("\r\nOK\r\n") >= 0;
  bool postRequested = actionStart.indexOf("+HTTPEXPOST") >= 0;
  if (!actionAccepted && !postRequested) {
    Serial.println(F("Air780EU did not acknowledge HTTPEXACTION."));
    endAir780HttpSession(true);
    return false;
  }

  // Give the documented +HTTPEXPOST URC a short opportunity to arrive after
  // the OK. If it is absent, continue with the data command anyway. Practical
  // Air780E AT implementations use the accepted HTTPEXACTION OK as sufficient
  // permission to request the '>' data prompt.
  if (!postRequested) {
    String requestSignal = cellReadUntil(1500, "+HTTPEXPOST", "\r\nERROR\r\n",
                                         false, "+CME ERROR:");
    if (requestSignal.indexOf("\r\nERROR\r\n") >= 0 ||
        requestSignal.indexOf("+CME ERROR:") >= 0) {
      Serial.println(F("Air780EU reported an error while preparing POST data."));
      endAir780HttpSession(true);
      return false;
    }
    postRequested = requestSignal.indexOf("+HTTPEXPOST") >= 0;
  }
  if (!postRequested) {
    Serial.println(F("Air780EU +HTTPEXPOST URC not seen; requesting POST prompt after HTTPEXACTION OK."));
  }

  CellSerial.print("AT+HTTPEXPOST=");
  CellSerial.print(json.length());
  CellSerial.print(',');
  CellSerial.print(AIR780_HTTP_POST_CHUNK_TIMEOUT_MS);
  CellSerial.print("\r\n");
  String postPrompt = cellReadUntil(5000, ">", "\r\nERROR\r\n", false,
                                    "+CME ERROR:");
  if (postPrompt.indexOf('>') < 0) {
    Serial.println(F("Air780EU extended HTTP service did not open the POST data prompt."));
    endAir780HttpSession(true);
    return false;
  }

  CellSerial.print(json);
  String postAccepted = cellReadUntil(AIR780_HTTP_POST_CHUNK_TIMEOUT_MS + 2000UL,
                                      "+HTTPEXPOST:", "\r\nERROR\r\n", false,
                                      "+CME ERROR:");
  if (postAccepted.indexOf("+HTTPEXPOST:") < 0) {
    Serial.println(F("Air780EU did not acknowledge the extended POST data chunk."));
    recordApplicationTransfer("cellular", json.length(), 0);
    endAir780HttpSession(true);
    return false;
  }
  int acknowledgedAt = postAccepted.lastIndexOf("+HTTPEXPOST:");
  if (postAccepted.indexOf('\n', acknowledgedAt) < 0) {
    postAccepted += cellReadUntil(1200, "\n", "ERROR\r\n");
  }
  int acknowledgedEnd = postAccepted.indexOf('\n', acknowledgedAt);
  String acknowledgedLength = postAccepted.substring(
    acknowledgedAt + strlen("+HTTPEXPOST:"),
    acknowledgedEnd >= 0 ? acknowledgedEnd : static_cast<int>(postAccepted.length()));
  acknowledgedLength.trim();
  if (acknowledgedLength.toInt() != static_cast<int>(json.length())) {
    Serial.println(F("Air780EU acknowledged an unexpected extended POST data length."));
    recordApplicationTransfer("cellular", json.length(), 0);
    endAir780HttpSession(true);
    return false;
  }
  ...
  bool resultReceived = readAir780ExtendedPostResult(responseBody, statusCode,
                                                      bearerDeactivated);
  responseBody.trim();

  // Count exact JSON body bytes handed to the modem and returned by HTTPEXGET.
  ...
  if (!resultReceived) {
    Serial.print(F("No complete HTTPEXACTION result after the configured modem timeout ("));
    Serial.print(AIR780_HTTP_TIMEOUT_SECONDS);
    Serial.println(F(" seconds)."));
    if (bearerDeactivated) {
      Serial.println(F("Air780E bearer deactivated during the HTTP request; cellular will reconnect."));
      cellReady = false;
    }
    endAir780HttpSession(true);
    return false;
  ...
  if (bearerDeactivated) {
    Serial.println(F("Air780E bearer deactivated during HTTPEXACTION; cellular will reconnect after this result."));
    cellReady = false;
  }
  ...
  if (!ok) {
    ...
    if (statusCode == 605) {
      // 605 is the Air780E HTTP service's SSL-channel-establishment failure,
      // not an HTTP response from Supabase. Retry after one clean context
      // application; repeated failure on V1180 points to modem TLS firmware.
      if (air780TlsConfigurationAttempts < 2) {
        ...
        Serial.println(F("Air780EU V1180 still cannot negotiate TLS. Update the modem firmware before enrollment."));
      }
    }
  }
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.20 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.20 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
