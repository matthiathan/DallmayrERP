...
static const char* FIRMWARE_VERSION = "6.8.20-esp32s3-actual-transport-usage";
...
bool airHttpPost(const char* url, const String& json, String& responseBody, int& statusCode, bool withDeviceAuth = true) {
  ...
  // reached Supabase. The extended sequence asks for each POST chunk explicitly
  // and avoids that modem-side buffered upload path.
  ...
  CellSerial.print("\r\n");
  String postRequest = cellReadUntil(20000, "+HTTPEXPOST\r\n", "\r\nERROR\r\n",
                                     false, "+CME ERROR:");
  if (postRequest.indexOf("+HTTPEXPOST\r\n") < 0) {
    Serial.println(F("Air780EU extended HTTP service did not request POST data."));
    endAir780HttpSession(true);
    return false;
  }
...
void addDataUsageMetadata(JsonDocument& doc, const char* transport) {
  // The same document is reused when the preferred network fails and the
  // request falls back to the other transport. Remove the previous attempt's
  // counters first so cellular modem totals can never be stored as Wi-Fi usage
  // (or vice versa).
  doc.remove("data_usage");
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.20 ACTUAL TRANSPORT USAGE DB-POLICY NATIVE MDB + DEX ---"));
