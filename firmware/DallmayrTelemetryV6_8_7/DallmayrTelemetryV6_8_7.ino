/*
  Dallmayr South Africa - Telemetry V6.8.7 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.7-esp32s3-air780eu-https-fix";
...
bool cellCommand(const String& command, const char* expected = "OK",
  ...
  if (command.startsWith("AT+HTTPPARA=\"USERDATA\"")) Serial.println(F("AT+HTTPPARA=\"USERDATA\",<redacted>"));
...
bool airHttpPost(const char* url, const String& json, String& responseBody, int& statusCode, bool withDeviceAuth = true) {
  ...
  // Both headers are retained because this exact gateway request shape was
  // already proven over the same Vodacom/Air780EU cellular path.
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.6 WIFI CONFIG-COMMIT DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.7 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
