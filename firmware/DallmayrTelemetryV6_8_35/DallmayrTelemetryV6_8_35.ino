/*
  Dallmayr South Africa - Telemetry V6.8.35 DB-POLICY NATIVE MDB + DEX
...
// Leave blank on already-enrolled devices: the provisioned Supabase key is
// loaded from NVS. If manually embedding a fallback, it must remain on ONE
// source line inside a complete C++ string literal: "..."
...
static const char* FIRMWARE_VERSION = "6.8.35-esp32s3-air780eu-ussd-auto-register";
...
// Keep the MCU wait longer than the modem deadline. V6.8.35 deliberately uses
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.35 AUTO-REGISTER VODACOM USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.35 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/MANUAL-PPPOS-BOOTSTRAP"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
