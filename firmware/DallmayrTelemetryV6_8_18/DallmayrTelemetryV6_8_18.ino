/*
  Dallmayr South Africa - Telemetry V6.8.18 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.18-esp32s3-air780eu-direct-ussd";
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.18 DIRECT VODACOM *111*502# USSD DB-POLICY NATIVE MDB + DEX ---"));
...
void handleConsoleLine(String line) {
  ...
  if (line.equalsIgnoreCase("VODACOM BALANCE")) {
    if (!cellReady) {
      Serial.println(F("Vodacom balance query requires a registered cellular modem."));
    } else {
      queryVodacomPrepaidBalance();
    }
    return;
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.18 - DB-POLICY NATIVE PASSIVE MDB + DEX + WIFI/CELLULAR"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
