/*
  Dallmayr South Africa - Telemetry V6.8.45 DB-POLICY NATIVE MDB + DEX
...
static const char* FIRMWARE_VERSION = "6.8.45-esp32s3-air780eu-durable-counters";
...
// Keep the MCU wait longer than the modem deadline. V6.8.45 deliberately uses
...
void incrementLocalVend(const String& selection, uint32_t priceCents, bool success) {
  ...
  c->dirty = true;
  counterStorageDirty = true;
  saveCountersNow();  // durable before the machine can lose power
  if (String(policy.mode).equalsIgnoreCase("live")) counterUploadRequested = true;
}
...
void printStatus() {
  Serial.println(F("--- Dallmayr Telemetry V6.8.45 AUTO-PROFILE CUP-COUNTERS DB-POLICY NATIVE MDB + DEX ---"));
...
void setup() {
  ...
  delay(2000);
  Serial.println();
  Serial.println(F("Dallmayr Telemetry V6.8.45 - PASSIVE MDB + DEX + AUTO-PROFILE EVIDENCE + CUP COUNTERS + WIFI/MANUAL PPPOS"));
#if defined(ARDUINO_USB_CDC_ON_BOOT) && ARDUINO_USB_CDC_ON_BOOT
