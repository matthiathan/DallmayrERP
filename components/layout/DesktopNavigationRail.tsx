...
export function DesktopNavigationRail({
  ...
  const uniqueQuickModules = quickModules.filter((item, index, items) => items.findIndex((candidate) => candidate.href === item.href) === index);
  const excludedHrefs = new Set([dashboardPath, homePath, '/work', ...uniqueQuickModules.map((item) => item.href)]);
  ...
  return (
    <aside aria-label="Application navigation" className={`dallmayr-sidebar ${collapsed ? 'is-collapsed' : ''}`}>
      ...
      <nav className="dallmayr-sidebar-nav" aria-label="ERP modules">
        ...
        {uniqueQuickModules.length ? <div className="dallmayr-sidebar-separator" aria-hidden="true" /> : null}

        <div className="dallmayr-sidebar-modules">
          {uniqueQuickModules.map((item) => (
            <Link
