...
export function EnterpriseServiceJobBoard() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      const text = [
        job.id, job.incident_number, job.job_number, job.ticket_case_number, job.ticket_reference,
        job.work_order_number, job.summary, job.complaint_details, customer, job.customer_code_snapshot,
        job.contact_name, job.telephone, job.mobile, job.contact_email, job.service_type,
        job.service_code, job.site_location, job.call_type, job.call_reason, job.category,
        job.sub_category, job.group_3, machine?.machine_name, machine?.serial_number, technician,
      ].join(' ').toLowerCase();
  ...
  return (
    <div className="grid spatial-stage spatial-dashboard service-call-log-stage">
      ...
      <PageToolbar
        ...
        description="View, search, assign and progress request-only service calls. Recurring plans and their complete operational details are created under Preventive Maintenance."
          ...
          <section className="card spatial-card service-call-log-lane" key={group.status}>
            ...
            <div className="grid">
              ...
              {group.jobs.map((job) => {
                ...
                const requirements = [job.parts_extra ? 'Parts extra' : null, job.performance_report_required ? 'Performance report' : null, job.visits_chargeable ? 'Chargeable visit' : null, job.quotation_required ? 'Quotation required' : null].filter(Boolean) as string[];
                return (
                  <article className="neo-card service-call-log-card" key={job.id}>
                    <div className="page-toolbar-heading"><div><strong>Incident {job.incident_number}</strong><small>{job.job_number}</small></div><StatusBadge value={job.priority} /></div>
