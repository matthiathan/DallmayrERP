...
export function StockControlPanel({ onCommitted }: { onCommitted?: () => void }) {
  ...
  async function resolveBarcode(value: string) {
    ...
    const { data, error: lookupError } = await getSupabaseClient()
      .from('stock_items')
      .select('id, stock_name, item_barcode, box_barcode, item_quantity, box_quantity, items_per_box, reorder_level, warehouse_location, default_location_id')
  ...
  async function commitTransaction(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(null);
    const client = getSupabaseClient();
    const { data, error: transactionError } = await client.rpc('apply_stock_transaction', {
  ...
  return (
    <section className="neo-card stock-control-centre">
      <div className="page-toolbar-heading">
        <div><div className="badge">Phone stock control</div><h2>Scan and transact stock</h2><p>{selectedMode.helper}</p></div>
        <StatusBadge value={stock ? 'active' : 'unknown'} label={stock ? 'Item matched' : 'Awaiting scan'} />
      </div>

      <div className="stock-mode-grid" role="tablist" aria-label="Stock transaction type">
        {modes.map((item) => <button aria-selected={mode === item.value} className={`stock-mode-button ${mode === item.value ? 'is-active' : ''}`} key={item.value} onClick={() => { setMode(item.value); setError(null); setMessage(null); }} role="tab" type="button"><strong>{item.label}</strong><span>{item.helper}</span></button>)}
      </div>
      ...
      <form className="grid" onSubmit={commitTransaction}>
        <div className="form-grid">
          ...
          <label>Quantity unit<select value={quantityUnit} onChange={(event) => setQuantityUnit(event.target.value as QuantityUnit)}><option value="item">Items</option><option value="box">Boxes</option></select></label>
          <label>{mode === 'cycle_count' ? 'Counted quantity' : 'Quantity'}<input min={mode === 'cycle_count' ? 0 : 1} type="number" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /></label>
        </div>
