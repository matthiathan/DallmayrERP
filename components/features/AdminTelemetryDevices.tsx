...
export function AdminTelemetryDevices() {
  ...
  const prepaidBalanceUnsupported = selectedPrepaid?.query_status === 'unsupported_modem_firmware';
    ...
    <section className={`fleet-route-page device-management-workspace ${selectedDevice ? 'has-device-detail' : ''}`}>
      ...
      {selectedDevice ? <aside className="device-detail-panel" aria-label={`${selectedDevice.device_code} configuration`}>
        ...
        <section className={`device-prepaid-balance is-${selectedPrepaid?.alert_level ?? 'unknown'}`}>
          ...
          <button className="fleet-button secondary" disabled={prepaidBalanceUnsupported || checkingBalance || selectedPrepaid?.request_pending} onClick={requestBalanceCheck} type="button">{prepaidBalanceUnsupported ? 'Carrier balance unavailable' : checkingBalance ? 'Queuing…' : selectedPrepaid?.request_pending ? 'Balance check queued' : 'Check balance now'}</button>
          <p>{prepaidBalanceUnsupported ? 'Air780EU V1180 does not support the USSD command required for a live Vodacom balance query. Use the device data-usage totals here and Vodacom self-service for the authoritative remaining balance.' : 'A supported carrier balance query is reported with the next heartbeat. Top up when this status becomes low or critical.'}</p>
        </section>
        ...
        <section><h3>Device information</h3><dl><div><dt>Firmware</dt><dd>{selectedDevice.firmware_version ?? 'Not reported'}</dd></div><div><dt>Network</dt><dd>{selectedDevice.last_transport ?? 'Not reported'}</dd></div><div><dt>Signal</dt><dd><SignalStrengthIndicator cellularCsq={selectedDevice.cellular_csq} transport={selectedDevice.last_transport} wifiRssi={selectedDevice.wifi_rssi} /></dd></div><div><dt>Last upload</dt><dd>{formatDate(selectedDevice.last_upload_at)}</dd></div><div><dt>Sequence</dt><dd>{selectedDevice.last_sequence.toLocaleString('en-ZA')}</dd></div></dl></section>
        ...
        <section className="device-data-usage">
          ...
          <div className="device-data-usage-summary">
            ...
            <div className={`is-${selectedPrepaid?.alert_level ?? 'unknown'}`}><span>Prepaid remaining</span><strong>{prepaidBalanceUnsupported ? 'Unavailable' : selectedPrepaid?.remaining_bytes === null || selectedPrepaid?.remaining_bytes === undefined ? 'Awaiting check' : formatBytes(selectedPrepaid.remaining_bytes)}</strong><small>{prepaidBalanceUnsupported ? 'Air780EU V1180 USSD unsupported' : selectedPrepaid?.request_pending ? 'Check queued' : selectedPrepaid?.checked_at ? `Checked ${formatDate(selectedPrepaid.checked_at)}` : 'No balance received yet'}</small></div>
          </div>
