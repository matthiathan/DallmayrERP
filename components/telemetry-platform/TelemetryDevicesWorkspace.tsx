...
import tabStyles from './DeviceWorkspaceTabs.module.css';
...
type DeviceWorkspaceTab = 'overview' | 'assignment' | 'connectivity' | 'mdb' | 'location' | 'sim' | 'advanced';
...
const DEVICE_TABS: Array<{ id: DeviceWorkspaceTab; label: string }> = [
...
export function TelemetryDevicesWorkspace() {
  ...
  const [activeTab, setActiveTab] = useState<DeviceWorkspaceTab>('overview');
  ...
  return <section className={styles.workspace} data-telemetry-devices="v3">
    ...
    {selected ? <aside className={styles.drawer} aria-label={`Manage ${selected.device_code}`}>
      ...
      <div className={styles.drawerBody}>
        <nav className={tabStyles.tabs} aria-label="Device settings sections" role="tablist">
          {DEVICE_TABS.map((tab) => <button aria-selected={activeTab === tab.id} className={`${tabStyles.tab} ${activeTab === tab.id ? tabStyles.active : ''}`} key={tab.id} onClick={() => setActiveTab(tab.id)} role="tab" type="button">{tab.label}</button>)}
        </nav>

        {activeTab === 'overview' ? <div data-device-tab="overview">
          ...
          <section className={styles.controlSection}><div className={styles.sectionTitle}><span>Aggregation</span><h3>Reporting</h3></div><label className={styles.field}><span>Counter mode</span><select value={reportingMode} onChange={(event) => setReportingMode(event.target.value as TelemetryMode)}><option value="live">Live</option><option value="daily">Daily</option><option value="monthly">Monthly</option></select></label></section>
          <TelemetryConfigSyncPanel key={`${selected.id}:${selected.updated_at}`} deviceId={selected.id} />
        </div> : null}

        {activeTab === 'assignment' ? <div data-device-tab="assignment"><section className={styles.controlSection}><div className={styles.sectionTitle}><span>Assignment</span><h3>Machine link</h3></div><div className={styles.currentMachine}>{machineLabel(currentMachine)}</div><form className={styles.machineSearch} onSubmit={searchMachines}><input placeholder="Machine name, serial or asset tag" value={machineSearch} onChange={(event) => setMachineSearch(event.target.value)} /><button disabled={searchingMachines} type="submit">{searchingMachines ? 'Searching…' : 'Find'}</button></form>{machineOptions.length ? <select value={machineId} onChange={(event) => setMachineId(event.target.value)}><option value="">Unassign device</option>{machineOptions.map((machine) => <option key={machine.id} value={machine.id}>{machineLabel(machine)}</option>)}</select> : <button className={styles.unassign} onClick={() => setMachineId('')} type="button">Clear assignment</button>}</section></div> : null}

        {activeTab === 'connectivity' ? <div data-device-tab="connectivity"><section className={styles.controlSection}><div className={styles.sectionTitle}><span>Connectivity</span><h3>Transport control</h3></div><label className={styles.field}><span>Preferred network</span><select value={transport} onChange={(event) => setTransport(event.target.value as TransportPreference)}><option value="auto">Auto · Wi-Fi first</option><option value="wifi">Wi-Fi preferred</option><option value="cellular">Cellular preferred</option></select></label><div className={styles.switchRow}><label><input checked={wifiEnabled} onChange={(event) => setWifiEnabled(event.target.checked)} type="checkbox" /><span>Wi-Fi enabled</span></label><label><input checked={cellularEnabled} onChange={(event) => setCellularEnabled(event.target.checked)} type="checkbox" /><span>Cellular enabled</span></label></div></section></div> : null}

        {activeTab === 'mdb' ? <div data-device-tab="mdb"><section className={styles.controlSection} data-mdb-pin-control="true"><div className={styles.sectionTitle}><span>Passive MDB</span><h3>Signal & pin order</h3></div><div className={styles.pinDiagram}><div className={!pinSwap ? styles.pinActive : undefined}><b>GPIO4</b><span>{pinSwap ? 'Master-RX' : 'Master-TX'}</span><small>{pinSwap ? 'Yellow / MDB pin 4' : 'Green / MDB pin 5'}</small></div><div className={pinSwap ? styles.pinActive : undefined}><b>GPIO5</b><span>{pinSwap ? 'Master-TX' : 'Master-RX'}</span><small>{pinSwap ? 'Green / MDB pin 5' : 'Yellow / MDB pin 4'}</small></div></div><div className={styles.segmented} role="group" aria-label="MDB passive pin order"><button aria-pressed={!pinSwap} className={!pinSwap ? styles.segmentActive : undefined} onClick={() => setPinSwap(false)} type="button">Standard</button><button aria-pressed={pinSwap} className={pinSwap ? styles.segmentActive : undefined} onClick={() => setPinSwap(true)} type="button">Swapped</button></div><div className={styles.twoFields}><label className={styles.field}><span>Master-TX polarity</span><select value={masterPolarity} onChange={(event) => setMasterPolarity(event.target.value as MdbPolarity)}><option value="auto">Auto detect</option><option value="normal">Normal</option><option value="inverted">Inverted</option></select></label><label className={styles.field}><span>Master-RX polarity</span><select value={slavePolarity} onChange={(event) => setSlavePolarity(event.target.value as MdbPolarity)}><option value="auto">Auto detect</option><option value="normal">Normal</option><option value="inverted">Inverted</option></select></label></div><p className={styles.safety}>GPIO4 and GPIO5 remain input-only. Changing pin order only changes which passive RMT input is decoded as Master-TX or Master-RX.</p></section></div> : null}

        {activeTab === 'location' ? <div data-device-tab="location"><section className={styles.controlSection}><div className={styles.sectionTitle}><span>Location</span><h3>Tracking cadence</h3></div><div className={styles.twoFields}><label className={styles.field}><span>Location interval</span><select value={locationInterval} onChange={(event) => setLocationInterval(Number(event.target.value))}><option value={1}>1 minute</option><option value={5}>5 minutes</option><option value={15}>15 minutes</option><option value={30}>30 minutes</option><option value={60}>1 hour</option><option value={1440}>Daily</option></select></label><label className={styles.field}><span>Movement threshold</span><select value={movementThreshold} onChange={(event) => setMovementThreshold(Number(event.target.value))}><option value={25}>±25 m</option><option value={50}>±50 m</option><option value={100}>±100 m</option><option value={250}>±250 m</option><option value={500}>±500 m</option></select></label><label className={styles.field}><span>Location override</span><input placeholder="Use assigned machine site by default" value={locationOverride} onChange={(event) => setLocationOverride(event.target.value)} /></label></div></section></div> : null}

        {activeTab === 'sim' ? <div data-device-tab="sim"><section className={styles.controlSection}><div className={styles.sectionTitle}><span>Vodacom prepaid</span><h3>SIM & data</h3></div><div className={styles.balanceHero}><div><span>Used · 30 days</span><strong>{formatBytes(selectedUsed)}</strong></div><div className={selectedPrepaid && ['low', 'critical', 'depleted'].includes(selectedPrepaid.alert_level) ? styles.balanceWarning : undefined}><span>Remaining</span><strong>{selectedPrepaid?.remaining_bytes != null ? formatBytes(selectedPrepaid.remaining_bytes) : 'Unavailable'}</strong></div></div><div className={styles.threeFields}><label className={styles.field}><span>Low warning · MB</span><input min={1} type="number" value={warningMb} onChange={(event) => setWarningMb(Number(event.target.value))} /></label><label className={styles.field}><span>Critical · MB</span><input min={0} type="number" value={criticalMb} onChange={(event) => setCriticalMb(Number(event.target.value))} /></label><label className={styles.field}><span>Check interval</span><select value={balanceInterval} onChange={(event) => setBalanceInterval(Number(event.target.value))}><option value={60}>1 hour</option><option value={180}>3 hours</option><option value={360}>6 hours</option><option value={720}>12 hours</option><option value={1440}>Daily</option></select></label></div><button className={styles.secondaryAction} disabled={checkingBalance || selectedPrepaid?.request_pending || selectedPrepaid?.query_status === 'unsupported_modem_firmware'} onClick={() => void checkBalance()} type="button">{selectedPrepaid?.query_status === 'unsupported_modem_firmware' ? 'Carrier balance unavailable' : selectedPrepaid?.request_pending ? 'Balance check queued' : checkingBalance ? 'Queuing…' : 'Check balance now'}</button><small>Vodacom billing remains authoritative. Air780EU firmware may not expose prepaid USSD balance queries.</small></section></div> : null}

        {activeTab === 'advanced' ? <div data-device-tab="advanced"><section className={styles.controlSection}><div className={styles.sectionTitle}><span>Administration</span><h3>Advanced device controls</h3></div><label className={styles.field}><span>Controller state</span><select value={deviceStatus} onChange={(event) => setDeviceStatus(event.target.value as DeviceStatus)}><option value="active">Active</option><option value="disabled">Disabled</option></select></label><dl className={styles.audit}><div><dt>Hardware UID</dt><dd>{selected.hardware_uid ?? 'Not reported'}</dd></div><div><dt>Profile</dt><dd>{selected.profile_id ?? 'Automatic'}</dd></div><div><dt>Config sent</dt><dd>{formatDate(selected.last_config_at)}</dd></div><div><dt>Config ACK</dt><dd>{formatDate(selected.last_config_ack_at)}</dd></div></dl><button className={styles.deleteButton} onClick={() => { setDeleteConfirmation(''); setDeleteOpen(true); }} type="button">Delete telemetry device</button></section></div> : null}
      </div>
