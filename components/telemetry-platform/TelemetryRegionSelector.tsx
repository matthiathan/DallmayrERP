...
type RegionSummaryRow = { region: TelemetryRegion; machines: number; devices: number; users: number };
...
export function TelemetryRegionSelector() {
  ...
  useEffect(() => {
    if (managerOpen) void reloadManager();
  ...
  const setUserRegion = async (userId: string, nextRegion: TelemetryRegion) => {
    ...
    setError(null);
    try {
      const { error: saveError } = await getSupabaseClient().rpc('set_user_telemetry_region', { p_user_id: userId, p_region: nextRegion });
      if (saveError) throw saveError;
      await reloadManager();
  ...
  if (!userDetails) return <div className={styles.chip}><NavigationIcon kind="pin" /><span>Region unavailable</span></div>;
      ...
      <div className={styles.selectorGroup}>
        {canManage ? (
          <label className={styles.selectChip}>
            ...
            <select aria-label="Current telemetry region" disabled={savingKey === 'self'} onChange={(event) => setMyRegion(event.target.value as TelemetryRegion)} value={region ?? ''}>
                ...
                return (
                  <article className={styles.assignmentRow} key={assignment.user_id}>
                    ...
                    <label>
                      ...
                      <select disabled={savingKey === assignment.user_id || !assignment.is_active} onChange={(event) => setUserRegion(assignment.user_id, event.target.value as TelemetryRegion)} value={assignment.telemetry_region ?? ''}>
...
export function TelemetryRegionRequired() {
  ...
  return (
    <section className={styles.required} aria-labelledby="telemetry-region-required-title">
      ...
      <p>Your telemetry region determines which machines, devices, faults, vending data, maps and Test Center sessions you can access. For normal users this choice is locked after selection.</p>
